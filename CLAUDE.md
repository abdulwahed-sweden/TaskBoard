...
### Notifications (email)
`tasks/notifications.py` emails the task's **assignee** (`task.owner`) on
assignment, comments, and status changes; the **actor is never notified about
their own action**, and a send is suppressed if the recipient has no email or
their `NotificationPreference` flag is off (per-event:
`notify_on_assignment` / `notify_on_comment` / `notify_on_status_change`, created
lazily via `get_or_create`, all default on). It hooks the same write paths as the
activity log — `on_task_created`/`on_task_updated` after `log_created`/
`log_changes` in `TaskCreateView`/`TaskUpdateView` and `TaskViewSet.perform_*`,
and `notify_comment` in `AddCommentView` (the importer self-suppresses since the
importer is the assignee). Sends are best-effort (failures logged, never raised).
Users edit toggles at `notification_preferences` (`NotificationPreferencesView`,
...
delivery via pytest-django's `mailoutbox` fixture. The same email config powers
the task notifications (see Notifications above).
...
as `tasks:task-list`, `tasks:task-detail`, etc.). Both viewsets are
...
**Auth & hardening (settings `REST_FRAMEWORK`):** auth is session **or** token
(`rest_framework.authtoken`); `POST /api/token/` exchanges username+password for
a token (`Authorization: Token <t>`); unauthenticated requests get `403`. List
endpoints are **paginated** (`PageNumberPagination`, `PAGE_SIZE=20` → `{count,
next, previous, results}`), and the task endpoint supports **filtering**
(`django-filter`: `?project=`, `?is_done=`, `?status=`), **ordering**
(`?ordering=created|due_date|title`), and **search** (`?search=` over
title/notes) — all applied on top of the org-scoped queryset, so they only ever
narrow what the user can already see. **OpenAPI** is generated by
`drf-spectacular`: schema at `/api/schema/`, Swagger UI at `/api/docs/`, ReDoc at
`/api/redoc/`. The viewsets set `queryset = …none()` + a `swagger_fake_view`
guard in `get_queryset` so schema generation never touches `request.user`.
